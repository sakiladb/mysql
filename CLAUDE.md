...
- **`SYFT_FILE_METADATA_SELECTION: none` on the SBOM step is required.** Syft's default file
  ...
  exceeds `actions/attest`'s **16 MiB predicate limit** and the step fails (this broke `v9.0.4`
  before the fix). It keeps the component (package) inventory and drops the heavy per-file data.
...
> build-push output. The single-job repos (this one, postgres, mariadb, sqlserver, clickhouse, rqlite)
